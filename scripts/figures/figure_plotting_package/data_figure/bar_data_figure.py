...
class BasicFluxErrorBarDataFigure(BasicBarDataFigure):
    ...
    def __init__(
        ...
        pathway_name_dict = default_parameter_extract(
            figure_data_parameter_dict, ParameterName.pathway_name_dict, None, pop=True)
        (
            default_x_tick_labels, pathway_separator_location_array, pathway_name_location_array,
            pathway_name_list
        ) = net_flux_x_axis_labels_generator(
            common_flux_name_list, group_id_name_dict=pathway_name_dict, with_glns_m=with_glns_m)
        input_x_tick_labels = default_parameter_extract(
            figure_data_parameter_dict, ParameterName.x_tick_labels_list, '', pop=True)
        if input_x_tick_labels == '':
            x_tick_labels = default_x_tick_labels
        else:
            x_tick_labels = input_x_tick_labels
